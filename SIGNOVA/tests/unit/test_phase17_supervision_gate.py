"""
Phase 17 Centralized Supervision Gate Unit Tests.
...
def test_phase17_orchestrator_dynamic_evaluation_without_hardcoding(tmp_path):
    """Dynamic evaluation with no annotations must naturally resolve to STATE_B."""
    empty_dir = tmp_path / "empty_pilot"
    empty_dir.mkdir()

    orchestrator = Phase17Orchestrator(annotations_dir=empty_dir)
    ...
    assert report["sample_accounting"]["total_discovered"] == 0
    assert report["human_data_present"] is False
    assert report["human_data_authenticated"] is False
