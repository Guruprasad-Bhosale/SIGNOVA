"""
Phase 17 Reference Repository Cryptographic Integrity Baseline Tests.

Verifies:
1. All 44/44 reference files in ISLTranslate-main and isl-translator-main match baseline SHA-256 hashes.
2. Read-only policy preservation across external codebases during Phase 17.
3. Phase 17 integrity report correctness.
"""
...
def test_reference_repositories_sha256_integrity_phase17():
...
def test_phase17_integrity_report_json():
    rep_path = Path("outputs/reports/phase17_integrity.json")
    assert rep_path.is_file(), "outputs/reports/phase17_integrity.json must exist"
