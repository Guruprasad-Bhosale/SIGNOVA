"""
Phase 19 Annotator Qualification Profile and Audit Unit Tests.

Validates:
- Qualification state transitions (UNQUALIFIED, QUALIFIED, SUSPENDED, REVOKED).
- Distinction between annotator and signer identities.
"""

from signova.annotation.annotator import (
    AnnotatorProfile,
    AnnotatorQualificationState,
    AnnotatorRole,
)
...
def test_annotator_profile_qualification_states():
    prof = AnnotatorProfile(
        annotator_id="ann_01",
        name="ISL Annotator 1",
        role=AnnotatorRole.ANNOTATOR,
        qualification_status=AnnotatorQualificationState.QUALIFIED,
    )
    d = prof.to_dict()
    assert d["qualification_status"] == "QUALIFIED"
