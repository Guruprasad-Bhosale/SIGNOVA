"""
Phase 15 Leakage Audit Unit Tests.
...
def test_leakage_audit_detects_duplicate_checksums():
    annots = [
        VideoAnnotation("p15_a1", "sample_01", "u1", False, ["A"], metadata={"source_checksum": "dup_sha_123"}),
        VideoAnnotation("p15_a2", "sample_02", "u1", False, ["A"], metadata={"source_checksum": "dup_sha_123"}),
    ]
...
def test_clean_annotations_pass_leakage_audit():
    annots = [
        VideoAnnotation("p15_a1", "sample_01", "u1", False, ["HELLO"], metadata={"source_checksum": "sha_unique_01"}),
        VideoAnnotation("p15_a2", "sample_02", "u2", False, ["WORLD"], metadata={"source_checksum": "sha_unique_02"}),
    ]
    report = audit_phase12_leakage(annots)
    assert len(report.duplicate_checksums) == 0
