"""
Phase 16 Preferred Split Hierarchy & Independence Constraint Unit Tests.
...
def test_split_hierarchy_prefers_signer_independent_when_signers_present():
    ...
    annots = [
        VideoAnnotation("p16_a1", "s1", "u1", False, ["A"], metadata={"signer_id": "signer_delhi_01"}),
        VideoAnnotation("p16_a2", "s2", "u2", False, ["B"], metadata={"signer_id": "signer_mumbai_02"}),
    ]
...
def test_split_hierarchy_prefers_session_independent_when_sessions_present():
    orch = Phase16Orchestrator()
    annots = [
        VideoAnnotation("p16_a1", "s1", "u1", False, ["A"], metadata={"session_id": "sess_recording_01"}),
        VideoAnnotation("p16_a2", "s2", "u2", False, ["B"], metadata={"session_id": "sess_recording_02"}),
    ]
    strat, rationale = orch._select_split_strategy(annots)
...
def test_split_hierarchy_falls_back_to_random_when_metadata_absent():
    orch = Phase16Orchestrator()
    annots = [
        VideoAnnotation("p16_a1", "s1", "u1", False, ["A"]),
        VideoAnnotation("p16_a2", "s2", "u2", False, ["B"]),
    ]
    strat, rationale = orch._select_split_strategy(annots)
