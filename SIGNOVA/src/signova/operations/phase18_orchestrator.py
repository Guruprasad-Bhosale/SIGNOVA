"""
Phase 18 Unified Orchestrator for SIGNOVA.

Coordinates:
- Human data collection workflow & provenance verification (HUMAN_DATA_AUTHENTICATED)
- Annotation lifecycle state enforcement (DRAFT -> SAVE -> SUBMIT -> REVIEW_PENDING -> VERIFIED / REJECTED)
...
- Configurable pilot thresholds & dataset scale classification:
    ...
    * DATA_LIMITED
    * RESEARCH_SCALE
- Pilot status taxonomy:
    * NOT_STARTED
    * IN_PROGRESS
    * BLOCKED_HUMAN_RESOURCE
    * COMPLETED
    * COMPLETED_INSUFFICIENT_DATA
- Dataset-level CTC feasibility:
    * T_required = L + sum I(y_i == y_{i+1})
    * T_features >= T_required checked across all samples
    * Summary: total, feasible, infeasible, feasibility_rate, min_frames, max_gloss_len, max_required_frames
...
- Canonical readiness evaluation engine (evaluate_phase18_readiness)
"""
...
class Phase18Orchestrator:
    """Orchestrates Phase 18 Genuine Human Annotation Execution, Dataset Qualification & CTC Gate."""
    ...
    def run_full_qualification(self) -> Dict[str, Any]:
        ...
        # 13. Pilot Status Determination (Distinguishing NOT_STARTED vs BLOCKED_HUMAN_RESOURCE vs IN_PROGRESS)
        drafts_dir = self.annotations_dir.parent / "drafts"
        has_draft_activity = drafts_dir.exists() and any(drafts_dir.glob("*.json"))
        ...
        # 14. Generalization Claims & Evaluation Readiness
        ...
        if human_data_qualified and training_eligible_count > 0:
            dataset_ver: Optional[str] = "phase18-sequential-isl-v1"
    ...
    def _evaluate_dataset_ctc_feasibility(
        ...
        for ann in annotations:
            ...
            repeats = sum(1 for i in range(L - 1) if ann.glosses[i] == ann.glosses[i + 1])
            T_req = L + repeats
            required_frames_list.append(T_req)

            # Determine available feature frames if landmark file exists
            T_feat = None
            ...
            if T_feat is not None:
                ...
                # If feature frames are unknown/unextracted, assess non-empty requirement
                if T_req > 0:
                    feasible += 1
                else:
                    infeasible += 1
...
def evaluate_phase18_readiness(
    workspace_root: Optional[Path] = None,
    annotations_dir: Optional[Path] = None,
    features_dir: Optional[Path] = None,
) -> Dict[str, Any]:
    """
    Canonical single-source-of-truth Phase 18 readiness evaluation function.
    Called by check_phase18.py, check_phase18_ctc_readiness.py, and run_phase18_verification.py.
    """
    ...
    model_path = root / "models" / "experiments" / "phase18_real_ctc" / "best_model.pt"
